...
# Clerk Publishable Key - Set this as environment variable or replace with your key
CLERK_PUBLISHABLE_KEY = 'pk_test_YnJhdmUtZ2hvc3QtNjUuY2xlcmsuYWNjb3VudHMuZGV2JA'
...
@app.route('/about')
def about():
    return render_template('about.html', clerk_key=CLERK_PUBLISHABLE_KEY)

@app.route('/services')
def services():
    return render_template('services.html', clerk_key=CLERK_PUBLISHABLE_KEY)


@app.route('/contact')
def contact():
    return render_template('contact.html', clerk_key=CLERK_PUBLISHABLE_KEY)
...
def chat():
    ...
    # If message is 'reset' or 'init', clear session and return welcome
    if user_msg and user_msg.strip().lower() in ['reset', 'init']:
        session.clear()
        session['step'] = 'welcome'
        return jsonify(reply="🤖 Welcome to MedFriend!\n👉 What is your name?")
    
    # If message is empty and step is welcome, return welcome message (but don't change step)
    if not user_msg and step == 'welcome':
        return jsonify(reply="🤖 Welcome to MedFriend!\n👉 What is your name?")
    
    # replicate each console step
    if step == 'welcome':
        # User sent a message, so move to name step
        session['step'] = 'name'
        # Process the message as the name
...
def final_prediction():
    ...
    text += f"\n\n\nThank you for using the MedFriend. Wishing you good health, {session['name']}!"
